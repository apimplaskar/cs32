...
//  hw4-sort
//
//  Created by Aditya Pimplaskar on 3/2/19.
...
bool compareStorePtr(const Store* lhs, const Store* rhs)
{
    ...
    bool comparison = compareStore(*lhs, *rhs); // just dereference and pass in
    return comparison;
}
...
void insertion_sort(vector<Store>& s, bool comp(const Store&, const Store&))
{
    ...
    // Translate the algorithm's check of >
    //s[i] > nextItem is the same as comp(nextItem, s[i]) being true
    
    //Followed the algorithm from the book
